...
const { getCourseData, changeField, changeCourseStatus } = require('../services/Course'),
      ...
      { getCoureFieldsData } = require('../services/CourseTab'),
      { returnInfo } = require('../libs/utils'),
      { API } = require('../config/error_config'),
      { getTeachers } = require('../services/Teacher');

class Index{
  async getCourses(ctx, next){
    const courseData = await getCourseData(),
          fieldData = await getCoureFieldsData();
  ...
  async changeTeacherStar(ctx, next){
    ...
    if(!result){
      ctx.body = returnInfo(API.SELECT_TEACHER_FAILED);
    }

    ctx.body = returnInfo(API.SELECT_TEACHER_SUCCESS);
  }
